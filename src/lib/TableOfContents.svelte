...
{#if items.length}
	<aside class="hidden xl:block">
		<div class="sticky top-20">
			<div class="mb-2 text-xs font-semibold text-muted-foreground">On this page</div>
			<ul class="space-y-1">
				{#each items as it}
					<li>
						<a
							href={`#${it.id}`}
							class="block rounded px-2 py-1 text-sm hover:bg-accent hover:text-accent-foreground {activeId ===
							it.id
