<script lang="ts">
  ...
  function copyAll() {
    try { navigator.clipboard.writeText(toolkitRaw); } catch {}
  }
  function downloadMd() {
...
<DocsLayout title="HeartCheck Viral Toolkit" description="Copy‑paste scripts, tweet templates, and memes to make ethics transparency go viral.">
  <div class="flex gap-2 mb-4">
    <button class={cn(buttonVariants({ variant: 'default', size: 'sm' }))} on:click={copyAll}>Copy all</button>
    <button class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} on:click={downloadMd}>Download .md</button>
  </div>
