...
<section class="py-8 md:py-12 lg:py-16">
	<div class="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8">
		<div class="mb-6 text-center">
			...
			{#if description}
				<p class="mx-auto mt-2 max-w-[70ch] text-muted-foreground">{description}</p>
			{/if}
		</div>
			...
			<aside class="hidden lg:block">
				<nav class="sticky top-20 space-y-1">
					{#each links as l}
						<a
							href={l.href}
							class="block rounded px-3 py-2 text-sm transition-colors hover:bg-accent hover:text-accent-foreground {path ===
							l.href
