...
<section class="space-y-6 py-8 md:py-12 lg:py-20">
  ...
  <div class="mx-auto w-full max-w-[58rem] px-4 sm:px-6 lg:px-8 space-y-8">
    <Implementation />
      ...
      <CardContent>
        ...
        {:else}
          <div class="flex flex-wrap gap-3 mb-3">
            <a class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} href="/heart.json" download>Download sample JSON</a>
            <button class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} on:click={() => copy(`curl -s https://heartcheck.dev/heart | jq`) }>Copy curl example</button>
            <button class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} on:click={() => download('heart.json', JSON.stringify(heart, null, 2))}>Download this response</button>
          </div>
