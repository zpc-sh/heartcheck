<script lang="ts">
  ...
  onMount(async () => {
    try {
      let res = await fetch('/heart', { headers: { accept: 'application/ld+json, application/json' } });
...
<section class="space-y-8 py-8 md:py-12 lg:py-16">
  <div class="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 grid gap-8 md:grid-cols-2">
    <div class="space-y-4">
      <h2 class="font-heading text-2xl sm:text-3xl">What is HeartCheck?</h2>
      <p class="text-muted-foreground">
        HeartCheck is a tiny, public <code class="px-1 rounded bg-muted">/heart</code> endpoint that returns JSON‑LD about a service’s ethical transparency —
        links to governance documents, a timestamp and version, plus optional self‑assessed metrics.
      </p>
    ...
    <Card>
      ...
      <CardContent>
        <pre class="overflow-x-auto rounded-lg bg-muted p-4 text-xs sm:text-sm"><code>{jsonExample}</code></pre>
      </CardContent>
    </Card>
  </div>
      ...
      <CardContent>
        ...
        {:else}
          <div class="flex flex-wrap gap-3 mb-3">
            <button class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} on:click={() => copy(JSON.stringify(heart, null, 2))}>Copy JSON</button>
            <button class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} on:click={() => copy(curlCmd)}>Copy curl</button>
            <a class={cn(buttonVariants({ variant: 'outline', size: 'sm' }))} href="/validate">Open validator</a>
          </div>
          <pre class="overflow-x-auto rounded-lg bg-muted p-4 text-xs sm:text-sm"><code>{JSON.stringify(heart, null, 2)}</code></pre>
        {/if}
      </CardContent>
    </Card>
  </div>

  <div class="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 grid gap-6 md:grid-cols-3">
    <Card>
      <CardHeader>
  ...
  <div class="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 grid gap-8 md:grid-cols-[1fr_380px] items-start">
    <Card>
      <CardHeader>
      ...
      <CardContent>
        ...
        <pre class="overflow-x-auto rounded-lg bg-muted p-4 text-xs sm:text-sm"><code>{curlCmd}</code></pre>
      </CardContent>
    </Card>

    <div class="space-y-3">
      <h3 class="font-heading text-xl">Adopt in minutes</h3>
      <p class="text-sm text-muted-foreground">Generate a Cloudflare Worker that serves <code>/heart</code> — no servers, no databases, no backend work.</p>
      <div class="flex flex-wrap gap-3">
        <Button href="/builder" class="h-10 px-5">Open Builder</Button>
        <a class={cn(buttonVariants({ variant: 'ghost', size: 'sm' }))} href="/viral">Viral toolkit →</a>
      </div>
    </div>
  </div>
</section>
