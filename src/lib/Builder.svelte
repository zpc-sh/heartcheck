<script lang="ts">
  ...
  function buildHeart() {
    ...
    if (algorithmic_transparency !== '') base['algorithmic_transparency'] = Number(algorithmic_transparency);
    if (data_minimization_score !== '') base['data_minimization_score'] = Number(data_minimization_score);
  ...
  async function openWorkersNew() {
    ...
    try { await navigator.clipboard.writeText(worker); } catch {}
  ...
  async function testEndpoint() {
    testResult = 'Testing…';
    try {
      const res = await fetch(testUrl, { headers: { accept: 'application/ld+json, application/json' } });
      const ct = res.headers.get('content-type') || '';
      const text = await res.text();
      let data: any;
      try { data = JSON.parse(text); } catch { throw new Error('Response was not valid JSON'); }
      const required = ['@context','@type','@id','service_name','timestamp','version','governance'];
...
<section id="builder" class="space-y-6 py-8 md:py-12 lg:py-20">
  <div class="mx-auto w-full max-w-screen-xl px-4 sm:px-6 lg:px-8 flex flex-col items-center space-y-4 text-center">
    <h1 class="font-heading text-3xl leading-[1.1] sm:text-4xl md:text-5xl">Generate a Cloudflare Worker</h1>
    <p class="max-w-[85%] leading-normal text-muted-foreground sm:text-lg sm:leading-7">
      Fill out the form, preview JSON-LD, then one-click download a Worker and wrangler config to serve <code class="px-1 rounded bg-muted">/heart</code>.
    </p>
  </div>
    ...
    <Card>
      ...
      <CardContent class="space-y-4">
        <div>
          ...
          <Input id="origin" bind:value={origin} placeholder="https://example.com" class={errors.origin ? 'border-destructive' : ''} />
          <div class="flex gap-2 mt-2">
            {#each CANONICAL_DOMAINS as d}
              <button class="inline-flex items-center justify-center gap-2 rounded-md h-8 px-3 border bg-background hover:bg-accent hover:text-accent-foreground transition-colors text-xs" on:click={() => origin = d} type="button">Use {d.replace('https://','')}</button>
            {/each}
          </div>
          {#if errors.origin}<div class="text-xs text-destructive mt-1">{errors.origin}</div>{/if}
        </div>
        <div>
      ...
      <CardContent class="grid gap-6 md:grid-cols-3">
        <div class="space-y-3">
          <div>
            ...
            <Input id="purl" bind:value={privacy_url} class={errors.privacy_url ? 'border-destructive' : ''} />
            {#if errors.privacy_url}<div class="text-xs text-destructive mt-1">{errors.privacy_url}</div>{/if}
          </div>
          <div>
            <Label for="plast">Privacy lastModified</Label>
            <Input id="plast" bind:value={privacy_last} class={errors.privacy_last ? 'border-destructive' : ''} />
            {#if errors.privacy_last}<div class="text-xs text-destructive mt-1">{errors.privacy_last}</div>{/if}
          </div>
          <div>
        ...
        <div class="space-y-3">
          <div>
            ...
            <Input id="turl" bind:value={terms_url} class={errors.terms_url ? 'border-destructive' : ''} />
            {#if errors.terms_url}<div class="text-xs text-destructive mt-1">{errors.terms_url}</div>{/if}
          </div>
          <div>
            <Label for="tlast">Terms lastModified</Label>
            <Input id="tlast" bind:value={terms_last} class={errors.terms_last ? 'border-destructive' : ''} />
            {#if errors.terms_last}<div class="text-xs text-destructive mt-1">{errors.terms_last}</div>{/if}
          </div>
          <div>
        ...
        <div class="space-y-3">
          <div>
            ...
            <Input id="xurl" bind:value={transparency_url} class={errors.transparency_url ? 'border-destructive' : ''} />
            {#if errors.transparency_url}<div class="text-xs text-destructive mt-1">{errors.transparency_url}</div>{/if}
          </div>
          <div>
            <Label for="xlast">Transparency lastModified</Label>
            <Input id="xlast" bind:value={transparency_last} class={errors.transparency_last ? 'border-destructive' : ''} />
            {#if errors.transparency_last}<div class="text-xs text-destructive mt-1">{errors.transparency_last}</div>{/if}
          </div>
          <div>
      ...
      <CardContent>
        <div class="grid md:grid-cols-3 gap-4">
          <div>
            ...
            <Input id="dignity" type="number" min="0" max="1" step="0.01" bind:value={dignity_score} />
          </div>
          <div>
            <Label for="alg">algorithmic_transparency (0–1)</Label>
            <Input id="alg" type="number" min="0" max="1" step="0.01" bind:value={algorithmic_transparency} />
          </div>
          <div>
            <Label for="mini">data_minimization_score (0–1)</Label>
            <Input id="mini" type="number" min="0" max="1" step="0.01" bind:value={data_minimization_score} />
          </div>
        </div>
        <div class="grid md:grid-cols-3 gap-4 mt-4">
          <label class="flex items-center gap-2 text-sm"><input type="checkbox" bind:checked={everyone_benefits} /> everyone_benefits</label>
          <label class="flex items-center gap-2 text-sm"><input type="checkbox" bind:checked={consent_respected} /> consent_respected</label>
          <label class="flex items-center gap-2 text-sm"><input type="checkbox" bind:checked={explanation_available} /> explanation_available</label>
        </div>
      </CardContent>
    </Card>
  </div>
  ...
  <div class="mx-auto w-full max-w-[64rem] px-4 sm:px-6 lg:px-8 grid gap-6 md:grid-cols-2">
    <Card>
      ...
      <CardContent>
        <div class="flex flex-wrap gap-2 mb-3">
          <button class="inline-flex items-center justify-center gap-2 rounded-md h-9 px-3 border bg-background hover:bg-accent hover:text-accent-foreground transition-colors text-sm" on:click={() => navigator.clipboard.writeText(JSON.stringify(heart, null, 2))}>Copy JSON</button>
          <a class="inline-flex items-center justify-center gap-2 rounded-md h-9 px-3 border bg-background hover:bg-accent hover:text-accent-foreground transition-colors text-sm" href="/heart.json" download>Download sample JSON</a>
        </div>
    ...
    <Card>
      ...
      <CardContent class="space-y-3">
        <button class="w-full disabled:opacity-50" on:click={genWorker} disabled={hasErrors}>
          <span class={"inline-flex " + "w-full " + "justify-center " + "items-center " + "gap-2 " + "rounded-md " + "h-10 px-4 py-2 " + "bg-primary text-primary-foreground " + "hover:bg-primary/90 transition-colors text-sm font-medium"}>
            Download Worker + Wrangler
          </span>
        </button>
        <button class="w-full border rounded-md h-10 px-4 hover:bg-accent hover:text-accent-foreground transition-colors text-sm" on:click={openWorkersNew} disabled={hasErrors}>
          Open Cloudflare Editor (code copied)
        </button>
        <div class="text-xs text-muted-foreground">
          Quick deploy: create a new Worker in Cloudflare dashboard, paste <code>worker.js</code>, and publish. Or run <code>wrangler dev</code> locally.
        </div>
      </CardContent>
    </Card>
    <Card>
      ...
      <CardContent>
        <ol class="list-decimal pl-5 space-y-2 text-sm text-muted-foreground">
    ...
    <Card>
      ...
      <CardContent class="space-y-3">
        <div>
          ...
          <Input id="test" bind:value={testUrl} placeholder="https://your-worker.workers.dev/heart" />
        </div>
        <button class="inline-flex items-center justify-center gap-2 rounded-md h-9 px-3 border bg-background hover:bg-accent hover:text-accent-foreground transition-colors text-sm" on:click={testEndpoint} disabled={!testUrl}>
          Run test
        </button>
        {#if testResult}
    ...
    <Card>
      <CardHeader>
        ...
        <CardDescription>{DEPLOY_REPO_URL ? 'Deploy from the official template' : 'Paste your template repo URL'}</CardDescription>
      </CardHeader>
      <CardContent class="space-y-3">
        {#if DEPLOY_REPO_URL}
          <button class="inline-flex items-center justify-center gap-2 rounded-md h-10 px-4 bg-primary text-primary-foreground hover:bg-primary/90 transition-colors text-sm font-medium" on:click={deployViaRepo}>
        ...
        {:else}
          <div>
            ...
            <Input id="repo" bind:value={repoUrl} placeholder="https://github.com/YOU/heartcheck-template" />
          </div>
          <button class="inline-flex items-center justify-center gap-2 rounded-md h-9 px-3 border bg-background hover:bg-accent hover:text-accent-foreground transition-colors text-sm" on:click={deployViaRepo} disabled={!repoUrl}>
            Deploy to Cloudflare
          </button>
          <div class="text-xs text-muted-foreground">
            Use the <code>cloudflare-template/</code> provided in this project as a starting point. Put it in its own repo so this button works.
          </div>
        {/if}
      </CardContent>
    </Card>
